...
# Custom JSON encoder to handle datetime objects and OCI objects
    ...
    def default(self, obj):
        ...
        if hasattr(obj, '__dict__'):
            return {k:v for k,v in obj.__dict__.items() 
                    if not k.startswith('_') and k not in ['swagger_types', 'attribute_map']}
        try:
...
# Convert OCI model objects to proper dictionaries
def oci_object_to_dict(obj):
    """
    Convert OCI model objects to proper dictionaries with actual values.
    """
    # Handle None
    if obj is None:
        return None
    ...
    # Handle OCI dictionaries
    if isinstance(obj, dict):
        result = {}
        for key, value in obj.items():
            # Skip metadata fields completely
            if key not in ['swagger_types', 'attribute_map']:
                # Process nested dictionaries
                result[key] = oci_object_to_dict(value)
        return result
    
    # Handle OCI objects with direct attribute access
    if hasattr(obj, 'id') or hasattr(obj, 'name') or hasattr(obj, 'display_name'):
        result = {}
        # Try to get all potential attributes
        for attr in dir(obj):
            # Skip private attributes, callables, and metadata
            if (not attr.startswith('_') and 
                not callable(getattr(obj, attr)) and 
                attr not in ['swagger_types', 'attribute_map']):
                try:
                    value = getattr(obj, attr)
                    result[attr] = oci_object_to_dict(value)
                except:
                    pass
        return result
    ...
    if hasattr(obj, '__dict__'):
        ...
        for key, value in obj.__dict__.items():
            if not key.startswith('_') and key not in ['swagger_types', 'attribute_map']:
                result[key] = oci_object_to_dict(value)
        return result
...
def scan_oci_resources(compartment_id, config_path, profile_name):
    ...
    try:
        ...
        for bucket in buckets:
            ...
            try:
                ...
                if e.status != 404:  # Only print if it's not a 404 (Not Found) error
    ...
    try:
        ...
        for adb in autonomous_dbs:
            ...
            # Get backups - only use autonomous_database_id
            try:
                ...
                if e.status == 400:
                    adb_dict['backups'] = [{"error": "Unable to list backups - API error"}]
                else:
    ...
    try:
        ...
        for lb in load_balancers:
            ...
            for listener_name in lb.listeners:
                lb_dict['listeners_details'][listener_name] = oci_object_to_dict(lb.listeners[listener_name])
        ...
        for vault in vaults:
            ...
            try:
                ...
                try:
                    ...
                    # keys list is already initialized
            except Exception as e:
                print(f"Error processing vault {vault.id}: {e}")
                # keys list is already initialized
            
            # Always add the vault to the results
    ...
    try:
        secrets_client = oci.vault.VaultsClient(config)
    ...
    try:
        ...
        try:
            # Try with list_apis method first
            apis_response = api_gateway_client.list_apis(compartment_id=compartment_id)
            apis = apis_response.data.items if hasattr(apis_response.data, 'items') else []
            all_resources['api_gateways'] = [oci_object_to_dict(api) for api in apis]
        except Exception as e:
            ...
            try:
                ...
                for gateway in gateways:
                    ...
                    try:
                        ...
                        gateway_dict['deployments'] = []
                    
                    all_resources['api_gateways'].append(gateway_dict)
            except Exception as e:
                print(f"Could not list gateways either: {e}")
    except Exception as e:
